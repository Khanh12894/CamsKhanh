...
namespace XichLip.WebApi.Utilities
{
    public class FileUtils
    {
        ...
        public bool MergeFile(string FileName)
        {
            ...
            if (FilesList.Count() == FileCount)
            {
                ...
                //if (!MergeFileManager.Instance.InUse(baseFileName))
                {
                    ...
                    using (FileStream FS = new FileStream(baseFileName, FileMode.Create, FileAccess.Write))
                    {
        ...
        public string GetMergeFileName(string fileName)
        {
            string partToken = ".part_";
            if (fileName.IndexOf(partToken) < 0)
            {
        ...
        public string GetMergeFilePath(string fileName)
        {
            string partToken = ".part_";
            if (fileName.IndexOf(partToken) < 0)
            {
